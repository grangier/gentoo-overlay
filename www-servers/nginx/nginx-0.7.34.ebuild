...
IUSE="addition debug fastcgi flv imap pcre perl ssl status sub webdav zlib syslog wsgi eval strip"
...
src_unpack() {
        unpack ${A}
        cd "${S}"
	epatch "${FILESDIR}"/nginx-0.7.34.try_files.patch
	if use syslog; then
        	epatch "${FILESDIR}"/syslog-${PV}.patch
        fi
...
src_compile() {
	...
	use eval	&& myconf="${myconf} --add-module=${FILESDIR}/ngx_http_eval_module"
	use strip	&& myconf="${myconf} --add-module=${FILESDIR}/ngx_http_strip_filter_module"
