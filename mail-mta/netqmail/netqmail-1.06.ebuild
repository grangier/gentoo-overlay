...
IUSE="authcram gencertdaily highvolume mailwrapper qmail-spp ssl vanilla chkuser outgoingip outip"
RESTRICT="test"
...
src_unpack() {
	...
	if use chkuser; then
        	epatch ${FILESDIR}/netqmail-${PV}_chkuser-${CHKUSER}.patch
        	#doins ${FILESDIR}/chkuser_settings.h
            	echo -n "$(tc-getCC) ${CFLAGS} -I/home/vpopmail/include" > "${MY_S}/conf-cc"
        fi

	if use outgoingip; then
		epatch ${FILESDIR}/qmail-remote-outgoingip.diff
		einfo "Patching outgoingip";
	fi

	if use outip; then
		epatch ${FILESDIR}/qmail-1.03_outgoingips.patch
		einfo "PAtch outip"
	fi
}
